            ...
            <div class="row mb-3">
                ...
                <div class="col-md-9 d-flex justify-content-end pr-4">
                    <a href="{{ route('users.create') }}"
                        class="btn btn-primary btn-sm py-2 px-4">{{ __('Create User') }}</a>
                </div>
            </div>
            <div class="card my-2">
                            ...
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        @if ($user->generals->count() > 0)
                                            {{ $user->generals->first()->weight }}
                                        @endif
                                    </td>
                                    <td class="align-middle">
                                        @if ($user->generals->count() > 0)
                                            {{ $user->generals->first()->height }}
                                        @endif
                                    </td>
                                    <td class="align-middle">
                                        @if ($user->generals->count() > 0)
                                            {{ $user->generals->first()->gender }}
                                        @endif
                                    </td>
                                    <td class="align-middle">
                                        <div class="btn-group">
                                            <a href="{{ route('users.show', ['user' => $user['id']]) }}" type="button"
                                                class="btn btn-sm btn-success py-2 px-4">
                                                {{ __('Details') }}
                                            </a>
